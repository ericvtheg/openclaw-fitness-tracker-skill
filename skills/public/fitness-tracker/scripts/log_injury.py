...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Log a training constraint note (pain/illness/injury status). (Legacy name: log_injury.py)"
    )
    ...
    ap.add_argument("--severity", type=float, help="0-10 scale")
    ap.add_argument(
        "--status",
        default="active",
        choices=["active", "improving", "resolved", "flare"],
        help="Status tag",
    )
    args = ap.parse_args()
    ...
    conn.execute(
        """
        INSERT INTO constraints_entries (day, created_at, raw_text, area, severity, status)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
